...
const Lecture = ({user}) => {
 ...
 if(user && user.role!== "admin" && user.subscription.include(params.id)) 
    return navigate("/");
        ...
        <div className="right">
...

            {
                lectures && lectures.length > 0 ? 
                    lectures.map((e, i) => (
                        <>
                    <div onClick={() => fetchLecture(e._id)} key={i} className={`lecture-number ${lecture._id === e.id && "active"}`}>
                        {i + 1}.{e.title}
                    </div>
                    {
                        user && user.role==="admin" && <button className='common-btn' 
                        style={{background: "#db1313"}} onClick={()=>deleteHandler(e._id)} 
